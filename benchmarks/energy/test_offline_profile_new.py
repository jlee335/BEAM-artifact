...
NUM_TRIALS = 25  # Fixed number of iterations per configuration
NUM_TRIALS_LOW = 50 # Under 128 batch size, run 100 trials
...
def runner_func(batch_size, decode_size, total_ctx_len, model_executor, num_trials):
    ...
    print(f"Running with prefill_size_real: {prefill_size_real}, decode_size: {decode_size}, total_ctx_len: {total_ctx_len}, uniform_decode: {uniform_decode}, num_trials: {num_trials}")
...
def main(args: argparse.Namespace):
    ...
    file_name = f"{output_dir}/dvfs_profile_{gpu_name}_{model_name}_tp{tp_size}_pp{pp_size}_one.csv"
    ...
    if os.path.exists(file_name):
        ...
        for _, row in existing_df.iterrows():
            ...
            key = (row['clock'], row[batch_size_col], row['decode_tokens'], 
                   row['total_ctx_len'])
            # Only add if num_trials equals expected value (NUM_TRIALS or NUM_TRIALS_LOW)
            num_trials = row['num_trials']
            batch_size_val = row[batch_size_col]
            expected_trials = NUM_TRIALS_LOW if batch_size_val < 128 else NUM_TRIALS
    ...
    for total_ctx_len in total_ctx_lens:
        for batch_size in batch_size_list:
            ...
            for clock in clocks:
                ...
                # Determine number of trials based on batch size
                trials_to_run = NUM_TRIALS_LOW if batch_size < 128 else NUM_TRIALS
                ...
                num_trials, time_taken = run_n_times(
                    runner_func, 1, batch_size, num_decode_tokens,
                    total_ctx_len, model_executor, 1)
                torch.cuda.synchronize()
                time.sleep(0.1)
                ...
                # Run with appropriate number of trials
                num_trials, time_taken = run_n_times(
                    runner_func, trials_to_run, batch_size, num_decode_tokens,
                    total_ctx_len, model_executor, trials_to_run)
