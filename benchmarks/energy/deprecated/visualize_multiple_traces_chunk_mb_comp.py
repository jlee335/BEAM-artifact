...
def get_trace_alias(trace_name):
    ...
    """
    # Check for sensitivity study prefixes first
    if trace_name.startswith('A_'):
        return '2048'
    elif trace_name.startswith('B_'):
        return '512'
    elif trace_name.startswith('C_'):
        return '256'
    elif trace_name.startswith('D_'):
        return '128'
    elif trace_name.startswith('E_'):
        return '1'
    elif trace_name.startswith('F_'):
        return '2'
    elif trace_name.startswith('G_'):
        return '3'
    elif trace_name.startswith('H_'):
        return '4'
...
def load_multiple_traces(parent_directory, prefixes=None):
    """
    ...
    Args:
        ...
        prefixes: List of directory prefixes to filter (e.g., ['A_', 'B_', 'C_', 'D_'])
    ...
    for item in os.listdir(parent_directory):
        # If prefixes specified, filter by prefix
...
def compute_metrics_dataframe(traces_dict):
    """
    Compute all key metrics for traces and return as a pandas DataFrame.
    
    Args:
        traces_dict: Dictionary mapping trace names to TraceData objects
        
    Returns:
        DataFrame with metrics for all traces
    """
    ...
    for trace_name in trace_names:
        ...
        # Compute metrics (convert time from seconds to milliseconds)
        metrics = {
            'experiment': trace_alias,
            'trace_name': trace_name,
            'mean_ttft_ms': (np.mean(ttfts) * 1000) if len(ttfts) > 0 else 0,
            'p90_ttft_ms': (np.percentile(ttfts, 90) * 1000) if len(ttfts) > 0 else 0,
            'p99_ttft_ms': (np.percentile(ttfts, 99) * 1000) if len(ttfts) > 0 else 0,
            'mean_tbt_ms': (np.mean(tbts) * 1000) if len(tbts) > 0 else 0,
            'p90_tbt_ms': (np.percentile(tbts, 90) * 1000) if len(tbts) > 0 else 0,
            'p99_tbt_ms': (np.percentile(tbts, 99) * 1000) if len(tbts) > 0 else 0,
            'mean_tpot_ms': (np.mean(tpots) * 1000) if len(tpots) > 0 else 0,
            'p90_tpot_ms': (np.percentile(tpots, 90) * 1000) if len(tpots) > 0 else 0,
            'p99_tpot_ms': (np.percentile(tpots, 99) * 1000) if len(tpots) > 0 else 0,
            'pct_ttft_under_slo': pct_ttft_under_slo,
            'pct_tbt_under_slo': pct_tbt_under_slo,
            'pct_tpot_under_slo': pct_tpot_under_slo,
            'total_energy_joules': trace_data.energy_consumption,
            'total_duration_s': trace_data.total_duration,
            'avg_power_w': power_watts,
            'num_gpus': trace_data.num_gpus,
            'per_gpu_power_w': power_watts / trace_data.num_gpus,
            'num_requests': len(ttfts),
            'ttft_slo_ms': ttft_slo * 1000,
            'tbt_slo_ms': tbt_slo * 1000,
            'tpot_slo_ms': tpot_slo * 1000,
        }
...
if __name__ == "__main__":
    ...
    parser = argparse.ArgumentParser(
        description="Visualize trace data from benchmark results with separate chunk and microbatch comparisons."
    )
    parser.add_argument(
        "base_directory",
        type=str,
    ...
    parser.add_argument(
        "--mode",
        type=str,
        choices=["single", "multiple", "sensitivity", "auto"],
        default="auto",
        help="Visualization mode: 'single' for one trace, 'multiple' for comparing all traces, 'sensitivity' for chunk/microbatch sensitivity analysis, 'auto' to detect automatically"
    )
    ...
    if mode == "auto":
        ...
        if os.path.exists(traced_dataset_path):
            ...
            if has_trace_subdirs:
                # Check if this looks like a sensitivity study (has A_, B_, C_, D_, E_, F_, G_, H_ directories)
                dir_items = os.listdir(args.base_directory)
                if any(item.startswith('A_') for item in dir_items) and \
                   any(item.startswith('E_') for item in dir_items):
                    mode = "sensitivity"
                else:
                    mode = "multiple"
            else:
    ...
    if mode == "single":
        ...
        # Sensitivity analysis mode - separate chunk and microbatch comparisons
        print("\n" + "="*70)
        ...
        # ===== CHUNK SENSITIVITY (A, B, C, D) =====
        print("\n" + "-"*70)
        print("CHUNK SENSITIVITY COMPARISON (A, B, C, D)")
        print("-"*70)
        chunk_prefixes = ['A_', 'B_', 'C_', 'D_']
        chunk_traces = load_multiple_traces(args.base_directory, prefixes=chunk_prefixes)
        
        if chunk_traces:
            print(f"\nFound {len(chunk_traces)} chunk sensitivity traces:")
            for name in sorted(chunk_traces.keys()):
                print(f"  - {name}")
            
            # Generate distributions plot
            chunk_distributions_path = os.path.join(viz_dir, "chunk_sensitivity_distributions.png")
            print(f"\nGenerating chunk sensitivity distributions...")
            visualize_multiple_distributions(chunk_traces, save_path=chunk_distributions_path)
            
            # Generate bar comparison plot
            chunk_bars_path = os.path.join(viz_dir, "chunk_sensitivity_bars.png")
            print(f"\nGenerating chunk sensitivity bar comparison...")
            visualize_multiple_bars(chunk_traces, save_path=chunk_bars_path)
            
            # Compute and save metrics to CSV
            chunk_metrics_df = compute_metrics_dataframe(chunk_traces)
            chunk_csv_path = os.path.join(viz_dir, "chunk_sensitivity_metrics.csv")
            chunk_metrics_df.to_csv(chunk_csv_path, index=False)
            print(f"\nChunk sensitivity metrics saved to: {chunk_csv_path}")
            print("\nChunk Sensitivity Metrics Summary:")
            print(chunk_metrics_df[['experiment', 'mean_ttft_ms', 'mean_tbt_ms', 'mean_tpot_ms', 
                                   'avg_power_w', 'pct_ttft_under_slo', 'pct_tbt_under_slo']].to_string(index=False))
        else:
            print("\nNo chunk sensitivity traces found!")
        
        # ===== MICROBATCH SENSITIVITY (E, F, G, H) =====
        print("\n" + "-"*70)
        print("MICROBATCH SENSITIVITY COMPARISON (E, F, G, H)")
        print("-"*70)
        mb_prefixes = ['E_', 'F_', 'G_', 'H_']
        mb_traces = load_multiple_traces(args.base_directory, prefixes=mb_prefixes)
        
        if mb_traces:
            print(f"\nFound {len(mb_traces)} microbatch sensitivity traces:")
            for name in sorted(mb_traces.keys()):
                print(f"  - {name}")
            
            # Generate distributions plot
            mb_distributions_path = os.path.join(viz_dir, "microbatch_sensitivity_distributions.png")
            print(f"\nGenerating microbatch sensitivity distributions...")
            visualize_multiple_distributions(mb_traces, save_path=mb_distributions_path)
            
            # Generate bar comparison plot
            mb_bars_path = os.path.join(viz_dir, "microbatch_sensitivity_bars.png")
            print(f"\nGenerating microbatch sensitivity bar comparison...")
            visualize_multiple_bars(mb_traces, save_path=mb_bars_path)
            
            # Compute and save metrics to CSV
            mb_metrics_df = compute_metrics_dataframe(mb_traces)
            mb_csv_path = os.path.join(viz_dir, "microbatch_sensitivity_metrics.csv")
            mb_metrics_df.to_csv(mb_csv_path, index=False)
            print(f"\nMicrobatch sensitivity metrics saved to: {mb_csv_path}")
            print("\nMicrobatch Sensitivity Metrics Summary:")
            print(mb_metrics_df[['experiment', 'mean_ttft_ms', 'mean_tbt_ms', 'mean_tpot_ms', 
                                'avg_power_w', 'pct_ttft_under_slo', 'pct_tbt_under_slo']].to_string(index=False))
        else:
            print("\nNo microbatch sensitivity traces found!")
        
        print("\n" + "="*70)
        print(f"All visualizations and metrics saved to: {viz_dir}")
        print("="*70)
        ...
        # Compute and save metrics to CSV
        metrics_df = compute_metrics_dataframe(traces_dict)
        csv_path = os.path.join(viz_dir, "all_metrics.csv")
