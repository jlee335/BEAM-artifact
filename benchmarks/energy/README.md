# Energy-Inf V1 Disagg Benchmark Suite

This directory contains the scripts and configurations to fully reproduce the artifact evaluation results for our paper. A containerized environment is provided to ensure reproducible runs without contaminating your host system.

## Hardware & Software Requirements

- **GPU Setup**: 1–4 GPUs (A100/H100 recommended), depending on your target parallelism configuration. The `run_*.sh` scripts derive GPU count automatically via `nvidia-smi`.
- **System Requirements**:
...
2. **Start the Evaluator Container** — run from the **repository root**:
   ```bash
   cd ../../  # Ensure you are at the repository root
   docker compose up -d
   ```
...
**All benchmarks must be run inside the container.**
...
The following are approximate wall-clock times per step on 4× RTX A6000 GPUs with Qwen2.5-14B:
...
See [VISUALIZATION.md](VISUALIZATION.md) for full argument reference, expected directory layouts, and additional examples.
...
See [PROFILING.md](PROFILING.md) for a detailed description of each output file and the two-layer profiling architecture.
