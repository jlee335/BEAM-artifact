...
class AsyncLLM(EngineClient):
    ...
    async def start_profile(self, output_dir: str | None = None) -> None:
        coros = [self.engine_core.profile_async(True, output_dir)]
        if self.profiler is not None:
            coros.append(asyncio.to_thread(self.profiler.start, output_dir))
    ...
    async def update_chunk_size(self, chunk_size: int) -> None:
        print(f"AsyncLLM update_chunk_size {chunk_size}")
        coros = [self.engine_core.update_chunk_size_async(chunk_size)]
        await asyncio.gather(*coros)

    async def update_microbatch_size(self, microbatch_size: int) -> None:
        print(f"AsyncLLM update_microbatch_size {microbatch_size}")
        coros = [self.engine_core.update_microbatch_size_async(microbatch_size)]
        await asyncio.gather(*coros)

    async def update_tpot_slo(self, tpot_slo: float) -> None:
        print(f"AsyncLLM update_tpot_slo {tpot_slo}")
        coros = [self.engine_core.update_tpot_slo_async(tpot_slo)]
        await asyncio.gather(*coros)

    async def update_tbt_slo(self, tbt_slo: float) -> None:
        print(f"AsyncLLM update_tbt_slo {tbt_slo}")
        coros = [self.engine_core.update_tbt_slo_async(tbt_slo)]
        await asyncio.gather(*coros)

    async def update_ttft_slo(self, ttft_slo: float) -> None:
        print(f"AsyncLLM update_ttft_slo {ttft_slo}")
        coros = [self.engine_core.update_ttft_slo_async(ttft_slo)]
        await asyncio.gather(*coros)
