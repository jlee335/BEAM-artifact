...
class RequestState:
    def __init__(
        ...
        # Estimation fields
        self.estimated_ttft: Optional[float] = None
        self.estimated_tbts: list[float] = []
    ...
    def _new_request_output(
        ...
        # Extract estimations from first completion output (all outputs share same estimations)
        completion_outputs = cast(list[CompletionOutput], outputs)
        estimated_ttft = completion_outputs[0].estimated_ttft if completion_outputs else None
        # Accumulated TBT list from all previous tokens
        estimated_tbts = self.estimated_tbts.copy() if self.estimated_tbts else None
    ...
    def _new_completion_output(
        ...
        return CompletionOutput(
            ...
            estimated_ttft=self.estimated_ttft,
            estimated_tbts=self.estimated_tbts.copy() if self.estimated_tbts else None)
...
class OutputProcessor:
    ...
    def process_outputs(
        ...
        for engine_core_output in engine_core_outputs:
            ...
            if engine_core_output.estimated_ttft is not None:
                req_state.estimated_ttft = engine_core_output.estimated_ttft
            if engine_core_output.estimated_tbt is not None:
                req_state.estimated_tbts.append(engine_core_output.estimated_tbt)
