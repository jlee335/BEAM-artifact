...
class GPUModelRunner(
    ...
    def __init__(
        ...
        self.gpu_clock = NODVFS_GPU_CLOCK
        ...
        with (
            ...
            if self.vllm_config.parallel_config.custom_profiler:
                ...
                model_forward_time = model_forward_start.elapsed_time(
                    model_forward_end)
                self.profiler.append_log([
                    scheduler_output.step_cnt, scheduler_output.phase,
                    model_forward_time, batch_size, self.gpu_clock, num_reqs
                ])
    ...
    @torch.inference_mode()
    def _dummy_run_mixed(
        self,
        num_prefill_tokens: int,
        num_decode_tokens: int,
        total_decode_ctx_len: int,
        ...
        """
        Run a dummy forward pass with a mixed batch of prefill and decode requests.

        Args:
            num_prefill_tokens: Number of tokens in the single prefill request.
            num_decode_tokens: Number of decode tokens (each decode request has 1 token).
            total_decode_ctx_len: Total context length distributed equally among decode requests.
        ...
        for i in range(num_trials):
            ...
            # Only add prefill request if num_prefill_tokens > 0
            num_reqs = num_decode_tokens + (1 if num_prefill_tokens > 0 else 0)

            # Padding for DP
            num_pad, num_tokens_across_dp = self.get_dp_padding(num_tokens)
            num_tokens += num_pad

            # Validate against scheduler limits
            assert num_tokens <= self.scheduler_config.max_num_batched_tokens
            assert num_reqs <= self.scheduler_config.max_num_seqs

            # Create scheduled tokens list: each decode request has 1 token, prefill has num_prefill_tokens
            num_scheduled_tokens_list = [1] * num_decode_tokens
            if num_prefill_tokens > 0:
                num_scheduled_tokens_list.append(num_prefill_tokens)
            num_scheduled_tokens = np.array(num_scheduled_tokens_list, dtype=np.int32)

            # Max query length is the prefill tokens (longest query in batch), or 1 for decode-only
            max_query_len = num_prefill_tokens if num_prefill_tokens > 0 else 1

            # Always create attention metadata (as per requirements)
            attn_metadata = {}

            # Sequence lengths: decode requests have equal context, prefill has its tokens + context
            decode_seq_len = total_decode_ctx_len // num_decode_tokens if num_decode_tokens > 0 else 0
            
            seq_lens = [decode_seq_len] * num_decode_tokens
            if num_prefill_tokens > 0:
                prefill_seq_len = num_prefill_tokens + decode_seq_len  # Prefill sequence includes its tokens
                seq_lens.append(prefill_seq_len)
            ...
            # No LoRA support (as per requirements)
            ...
            # Extract logits for each request (last token of each request)
    ...
    def _dummy_run_mixed(
        ...
        for i in range(num_trials):
            ...
            if create_mixed_batch:
                ...
                num_reqs = min(num_prefill_tokens, max_num_reqs)
                min_tokens_per_req = num_prefill_tokens // num_reqs
                num_scheduled_tokens_list = [min_tokens_per_req] * num_reqs
                num_scheduled_tokens_list[-1] += num_prefill_tokens % num_reqs
            ...
            if force_attention or cudagraph_runtime_mode == CUDAGraphMode.FULL:
                # First, make seq_len list from decode by dividing total_ctx_len by num_decode_tokens
                seq_lens = [math.ceil(total_ctx_len / num_decode_tokens)] * num_decode_tokens
                if num_prefill_tokens > 0:
                    seq_lens.append(num_prefill_tokens + 1)
