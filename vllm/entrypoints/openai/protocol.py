...
class CompletionResponseChoice(OpenAIBaseModel):
    ...
    # vLLM-specific estimation fields
    estimated_ttft: float | None = None
    estimated_tbts: list[float] | None = None
...
class UpdateChunkSizeRequest(BaseModel):
    chunk_size: int
...
class UpdateMicrobatchSizeRequest(BaseModel):
    microbatch_size: int

    @field_validator("microbatch_size")
    @classmethod
    def validate_microbatch_size(cls, v: int) -> int:
        if v <= 0:
            raise ValueError("Microbatch size must be positive")
        return v


class UpdateTpotSloRequest(BaseModel):
    tpot_slo: float

    @field_validator("tpot_slo")
    @classmethod
    def validate_tpot_slo(cls, v: float) -> float:
        if v <= 0:
            raise ValueError("TPOT SLO must be larger than 0")
        return v


class UpdateTbtSloRequest(BaseModel):
    tbt_slo: float

    @field_validator("tbt_slo")
    @classmethod
    def validate_tbt_slo(cls, v: float) -> float:
        if v <= 0:
            raise ValueError("TBT SLO must be larger than 0")
        return v


class UpdateTtftSloRequest(BaseModel):
    ttft_slo: float

    @field_validator("ttft_slo")
    @classmethod
    def validate_ttft_slo(cls, v: float) -> float:
        if v <= 0:
            raise ValueError("TTFT SLO must be larger than 0")
        return v
