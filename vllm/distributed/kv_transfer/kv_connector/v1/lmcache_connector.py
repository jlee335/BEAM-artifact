...
from vllm.v1.worker.gpu_worker import get_global_energy_logger
...
class LMCacheConnectorV1(KVConnectorBase_V1):
    ...
    def start_load_kv(self, forward_context: "ForwardContext", **kwargs: Any) -> None:
        ...
        # Call the actual start_load_kv implementation
        ...
        # log only when over 0.1ms
        if time_taken_seconds > 0.00001:
            # Log to energy logger if available
            try:
                energy_logger = get_global_energy_logger()
                if energy_logger is not None:
                    energy_logger.log_start_load_kv(time_taken_seconds)
            except Exception as e:
                # Silently fail if energy logger is not available
                # to avoid disrupting normal operations
                logger.debug(f"Could not log to energy logger: {e}")
    ...
    def wait_for_layer_load(self, layer_name: str) -> None:
        ...
        # Call the actual wait_for_layer_load implementation
